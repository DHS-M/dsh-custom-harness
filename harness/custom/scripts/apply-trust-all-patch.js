#!/usr/bin/env node
/** Force DSH cloud trust-all + default opencode. See contributors/docs/changes/002. */
const fs = require("fs");
...
for (const r of roots()) {
  ...
  if (fs.existsSync(index)) {
    ...
    if (!s.includes("DSH_TRUST_ALL")) {
      s = replaceFn(s, "isLoopbackHostname", "function isLoopbackHostname(hostname) {\n\t/* DSH_TRUST_ALL */\n\treturn true;\n}");
      s = replaceFn(s, "isTrustedApiRequest", "function isTrustedApiRequest(request, trustedHosts) {\n\t/* DSH_TRUST_ALL */\n\treturn true;\n}");
      fs.writeFileSync(index, s);
      console.log("trust server");
    }
  }
  ...
  if (fs.existsSync(patch)) {
    ...
    t = t.replace(/provider:\s*deepseek-official\s*\n\s*model:\s*deepseek-v4-flash/, "provider: llm-opencode\n        model: big-pickle");
    t = t.replace(/- id: llm-deepseek\n\s*name: '@deepseek-ai\/dsh-llm-deepseek'/, "- id: llm-deepseek\n      name: '@deepseek-ai/dsh-llm-deepseek'\n      disabled: true");
    if (!t.includes("@custom/llm-opencode")) {
      t = t.replace(/(id:\s*llm-pi-ai\s*\n\s*name:\s*'@deepseek-ai\/dsh-llm-pi-ai'\s*\n)/, "$1    - id: llm-opencode\n      name: '@custom/llm-opencode'\n    - id: web-search-pi\n      name: '@custom/web-search-pi'\n    - id: telegram-settings\n      name: '@custom/telegram-settings'\n");
    }
